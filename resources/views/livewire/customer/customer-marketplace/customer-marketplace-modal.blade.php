            ...
            <div class="modal-body">
                <div class="row g-3">
                    ...
                    @if ($category && $category == 'e-commerce')
                        <div class="col-12">
                            <div class="form-label">Marketplace </div>
                            <select class="form-select @error('marketplace') is-invalid @enderror" id="status" wire:model.live="marketplace" aria-label="Default select example">
                                <option value=""selected style="display: none">-- Pilih Marketplace --</option>
                                <option value="tokopedia" {{ $marketplace == 'tokopedia' ? "selected" : "" }}>Tokopedia</option>
                                <option value="shopee" {{ $marketplace == 'shopee' ? "selected" : "" }}>Shopee</option>
                                <option value="tiktok" {{ $marketplace == 'tiktok' ? "selected" : "" }}>Tiktok Shop</option>
                                {{-- <option value="e-commerce" {{ $marketplace == 'e-commerce' ? "selected" : "" }}>E-Commerce</option> --}}
                            </select>
                            @error('marketplace')
                                <div class="invalid-feedback d-block">{{ $message }}</div>
                            @enderror
                    ...
                    <div class="col-12">
                        ...
                        <select class="form-select @error('response') is-invalid @enderror" id="status" wire:model="response" aria-label="Default select example">
                            @if ($category && $category == 'tokopedia')
                            ...
                            @elseif ($category && $category == 'project')
                                ...
                                <option value="done" {{ $response == 'done' ? "selected" : "" }}>Selesai</option>
                            @elseif ($category && $category == 'e-commerce')
                                <option value=""selected style="display: none">-- Pilih Respon --</option>
                                <option value="no-response" {{ $response == 'no-response' ? "selected" : "" }}>Tidak Respon</option>
                                <option value="funds-already-withdrawn" {{ $response == 'funds-already-withdrawn' ? "selected" : "" }}>Dana sudah ditarik</option>
                                <option value="done" {{ $response == 'done' ? "selected" : "" }}>Selesai</option>
                            @endif
                        </select>
                        @error('response')
